...
class Distribution(ABC):
    ...
    def get_actions(self, deterministic: bool = False, add_dirichlet_noise: bool = True) -> th.Tensor:
        """
        ...
        if deterministic:
            return self.mode()
        return self.sample(add_dirichlet_noise=add_dirichlet_noise)
    ...
    def log_prob(self, actions: th.Tensor, add_dirichlet_noise=True, add_epsilon=False) -> th.Tensor:
        ...
        if add_dirichlet_noise:
            ...
            if add_epsilon:
                ...
                logps = []
                for i, (dist, action) in enumerate(zip(self.distribution, th.unbind(actions, dim=1))):
                    original_probs = th.gather(dist.probs, 1, action.unsqueeze(1)).flatten()
                    logp = th.log(original_probs + epsilon)
                    # assert logp == dist.log_prob(action), "logp: {}, dist.log_prob(action): {}".format(logp, dist.log_prob(action))
                    # print("logp: ", logp)
                    # print("logp shape: ", logp.shape)
                    # print("logp new: ", dist.log_prob(action))
                    # print("logp new shape: ", dist.log_prob(action).shape)
                    logps.append(logp)
                return th.stack(logps, dim=1).sum(dim=1)
            else:
...
def make_proba_distribution(
    ...
    if isinstance(action_space, spaces.Box):
        ...
        print("Action distribution: the multi discrete action space is used!")
